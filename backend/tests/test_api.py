...
class TestExpenses:
    ...
    def test_add_expense_success(self, mock_supabase):
        ...
        mock_supabase.table.return_value.insert.return_value.execute.return_value = mock_expense_response
        
        mock_debtors_response = Mock()
        mock_debtors_response.data = [{"id": "debt123"}]
        mock_supabase.table.return_value.insert.return_value.execute.return_value = mock_debtors_response
...
class TestBalances:
    ...
    def test_get_group_balances_success(self, mock_supabase):
        ...
        mock_group_table.select.return_value.eq.return_value.single.return_value.execute.return_value = mock_group_response
        ...
        mock_persons_table.select.return_value.eq.return_value.execute.return_value = mock_persons_response
        
        mock_expenses_response = Mock()
        mock_expenses_response.data = [
            {"id": "exp1", "amount": 100, "payer_id": "person1"}
        ]
        mock_expenses_table.select.return_value.eq.return_value.execute.return_value = mock_expenses_response
        ...
        mock_debtors_table.select.return_value.in_.return_value.execute.return_value = mock_debtors_response
        ...
        def table_side_effect(table_name):
            ...
            elif table_name == "debtors_expenses":
