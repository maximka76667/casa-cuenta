...
import { getAllGroups, addPerson, submitExpense, getGroupPersons, getExpenses } from '../api'
...
describe('API functions', () => {
  ...
  describe('getAllGroups', () => {
    it('fetches groups successfully', async () => {
      ...
      mockedAxios.get.mockResolvedValue({ data: { groups: { data: mockGroups } } })
  ...
  describe('getGroupPersons', () => {
    it('fetches group persons successfully', async () => {
      ...
      mockedAxios.get.mockResolvedValue({ data: { persons: mockPersons } })
  ...
  describe('addPerson', () => {
    it('adds person with correct data', async () => {
      const mockResponse = { message: 'Person added', person: { id: '1', name: 'John' } }
      mockedAxios.post.mockResolvedValue({ data: mockResponse })

      const result = await addPerson('John', 'group1')

      expect(mockedAxios.post).toHaveBeenCalledWith('http://localhost:8000/persons', {
        name: 'John',
        group_id: 'group1'
      })
  ...
  describe('submitExpense', () => {
    it('submits expense with correct data', async () => {
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith('http://localhost:8000/expenses', {
        name: 'Test Expense',
        debtors: ['1', '2'],
        group_id: '1',
        amount: 50,
  ...
  describe('getExpenses', () => {
    it('fetches expenses for group successfully', async () => {
      ...
      mockedAxios.get.mockResolvedValue({ data: { expenses: { data: mockExpenses } } })
